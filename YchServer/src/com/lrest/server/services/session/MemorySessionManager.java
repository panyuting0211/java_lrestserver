...
public class MemorySessionManager implements SessionManagerInteface{
    ...
    public MemorySessionManager() {
        // 默认生成MemorySession之后 成功一个session
        session.put(Constants.GENERAL_SESSESSION_ID,
                  Constants.GENERAL_SESSESSION_ID + " "
                + Constants.GENERAL_SESSION_TOKEN + " "
                + Constants.MEMORY_SESSION_CURRENTTIME);
