...
import com.lrest.server.services.services.RedisPool;
import com.lrest.server.services.services.SystemManager;
...
public class RedisSessionManager implements SessionManagerInteface {
    ...
    public  String createSID(String _token,String _uid){
        ...
        RedisPool.getRedis().setex(SESSION_ID_PREFIX+sid+":INFO", Config.session_timeout*1000,info);
    ...
    public  String getSID(String _sid){
        ...
        if (info==null){
            ...
            if (System.currentTimeMillis()-Long.parseLong(info.substring(info.lastIndexOf(" ")+1,info.length()))<expirationUpdateInterval){
                ...
                RedisPool.getRedis().setex(SESSION_ID_PREFIX+_sid+":INFO", Config.session_timeout*1000,newinfo);
                return newinfo;
            }
